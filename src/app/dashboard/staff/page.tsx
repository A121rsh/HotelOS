...
import AddStaffModal from "@/components/AddStaffModal"; // ✅ Import Modal
import { deleteStaff } from "@/actions/staff"; // ✅ Import Delete Action
...
export default async function StaffPage() {
  ...
  const adminUser = await db.user.findUnique({
    ...
    include: { 
        ownedHotel: { 
            include: { 
                staff: { // ✅ Staff list fetch kar rahe hain (New Relation)
  ...
  // Security: Agar user Admin nahi hai, ya Hotel nahi hai
  if (!adminUser?.ownedHotel) {
      return <div className="p-8">Only Hotel Owners can access this page.</div>;
  }

  const staffList = adminUser.ownedHotel.staff;
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <Card className="border-l-4 border-l-slate-900 bg-slate-50/50">
                <CardHeader className="flex flex-row items-center justify-between pb-2">
                    <CardTitle className="text-lg font-bold flex items-center gap-2">
                         {adminUser.name} <span className="text-xs font-normal text-slate-400">(You)</span>
                    </CardTitle>
            ...
            {/* 2. STAFF LIST */}
            {staffList.map((staff) => (
                <Card key={staff.id} className={`border-l-4 shadow-sm
